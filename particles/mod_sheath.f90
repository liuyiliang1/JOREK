...
subroutine calc_E_sheath(node_list,element_list,edge_element_template,i_elm,x,st,Te,Ti,B,ne,E_sheath,phi_optional)
  ...
  use mod_interp
  use constants
  use phys_module,only: R_limiter,Z_limiter,n_limiter
...
phi_0=-3*Te  !the sample sheath potential 
lamuda=sqrt(EPS_ZERO*Te/(ne*EL_CHG)) !the debye length
R=0.5d0*sqrt(2*Ti*2*ATOMIC_MASS_UNIT*EL_CHG)/(norm2(B)*EL_CHG) !the radius of the particle
phi_1=0.25*phi_0
phi_2=0.75*phi_0
