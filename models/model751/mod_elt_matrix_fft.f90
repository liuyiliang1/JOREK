...
real*8     :: rhoimp0,rhoimp0_R,rhoimp0_Z,rhoimp0_p,rhoimp0_s,rhoimp0_t,rhoimp0_ss,rhoimp0_st,rhoimp0_tt,rhoimp0_RR,rhoimp0_ZZ,rhoimp0_RZ,rhoimp0_pp,rhoimp0_corr
  ...
  real*8     :: seed_source_val                                       !< seed island source at this Gauss point
...
!   -Slaved impurity concentration
real*8     :: rimp0_target, drimp0_target_dr0  ! target impurity density and derivative
        ...
        do mp = 1, n_plane
          ...
          do im=n_tor_start, n_tor_end
            ...
            if(with_impurities .and. slave_impurity)then
              ! --- Slaved impurity: relaxation to target concentration
              Pvec_prev(var_rhoimp) = v * delta_g(mp,var_rhoimp,ms,mt)
              Qvec_p(var_rhoimp) = -v * (rhoimp0 - rimp0_target) * slave_impurity_omega
              Qvec_k(var_rhoimp) = 0.d0
              ...
              do l=1,n_degrees
                ...
                do in =  n_tor_start, n_tor_end
                  ...
                  if(with_impurities .and. slave_impurity)then
                    ...
                    drimp0_target_dr0 = slave_impurity_conc &
                      / (slave_impurity_conc + (central_mass/m_imp) * (1.d0 - slave_impurity_conc * Z_imp))
                    Qjac_p(var_rhoimp, var_rhoimp) = -v * rhoimp * slave_impurity_omega
                    Qjac_p(var_rhoimp, var_rho)    = +v * rho * drimp0_target_dr0 * slave_impurity_omega
                    ! All other Jacobian entries remain zero (already initialized)
                  else if(with_impurities)then
                    Pjac   (var_rhoimp,var_rhoimp)  = v * rhoimp
...
subroutine impurities_modeling()
  ...
  ! --- Slaved impurity: compute target density from fixed concentration c = n_imp/n_e
  if (slave_impurity) then
    rimp0_target = slave_impurity_conc * rho0 &
      / (slave_impurity_conc + (central_mass/m_imp) * (1.d0 - slave_impurity_conc * Z_imp))
  end if
