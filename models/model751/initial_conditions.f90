...
if ( (my_id .eq. 0) .and. (n_order .le. 3) ) then
  ...
  do i=1,node_list%n_nodes
    ...
    if(with_impurities .and. slave_impurity)then
      ...
      node_list%node(i)%values(1,1,var_rhoimp) = slave_impurity_conc * node_list%node(i)%values(1,1,var_rho)
...
!---------------------------- seed island current initialization (ECCD-like)
if (my_id == 0) then
  do i = 1, node_list%n_nodes
    node_list%node(i)%values(:, :, var_jseed) = 0.d0
  enddo
  if (num_seed_islands > 0) then
    write(*,*) 'Model 751: seed islands driven by current equation (ECCD-like).'
    write(*,*) 'Number of seed islands:', num_seed_islands
    write(*,*) 'Seed current variable (var_jseed) initialized to zero.'
    if (seed_continuous) then
      write(*,*) 'Continuous injection mode (ECCD-like).'
    else
      write(*,*) 'One-time injection mode (tearing mode seed).'
    end if
    do iseed = 1, num_seed_islands
      write(*,'(A,I2,A,F8.4,A,F12.4,A,F8.4,A,I4,A,I4,A,F8.4)') &
