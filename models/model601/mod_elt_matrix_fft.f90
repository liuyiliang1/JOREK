...
real*8     :: jseed, jseed_x, jseed_y, jseed_p, jseed_s, jseed_t, jseed_ss, jseed_st, jseed_tt
...
real*8     :: jseed0, jseed0_s, jseed0_t, jseed0_p, jseed0_x, jseed0_y
real*8     :: Bgrad_jseed, Bgrad_jseed_psi, Bgrad_jseed_jseed
...
! --- Centrifugal force related variables
real*8     :: C0_cent, C0_cent_s, C0_cent_t, dC0_drho, dC0_dvpar
real*8     :: dC0_drho_s, dC0_drho_t, dC0_dvpar_s, dC0_dvpar_t

real*8     :: dV_dpsi_source(n_gauss,n_gauss), dV_dz_source(n_gauss,n_gauss)
real*8     :: BB2_x, BB2_y, BB2_s, BB2_t
...
real*8     :: dr0_corr_dn
real*8     :: rimp0_target, drimp0_target_dr0  ! slaved impurity target
    ...
    do ms=1, n_gauss
      do mt=1, n_gauss
        ...
        do mp = 1, n_plane
          ...
          ! --- Extract jseed variable (ECCD-style seed current)
          if (with_jseed) then
            jseed0    = eq_g(mp,var_jseed,ms,mt)
            jseed0_x  = (   y_t(ms,mt) * eq_s(mp,var_jseed,ms,mt) - y_s(ms,mt) * eq_t(mp,var_jseed,ms,mt) ) / xjac
            jseed0_y  = ( - x_t(ms,mt) * eq_s(mp,var_jseed,ms,mt) + x_s(ms,mt) * eq_t(mp,var_jseed,ms,mt) ) / xjac
            jseed0_p  = eq_p(mp,var_jseed,ms,mt)
            jseed0_s  = eq_s(mp,var_jseed,ms,mt)
            jseed0_t  = eq_t(mp,var_jseed,ms,mt)
          end if
          ...
          if ( with_TiTe ) then ! ******************************************************************
            ...
            if (with_impurities .and. slave_impurity) then
              rimp0_target = slave_impurity_conc * r0 / (slave_impurity_conc + (central_mass/m_imp) * (1.d0 - slave_impurity_conc * Z_imp))
            end if
          ...
          elseif (use_ics) then
            ...
            !> for now, set Zeff Jacobian derivatives to zero (kinetic ICS path does not provide them)
            dZ_eff_dT   = 0.d0
            dZ_eff_dr0  = 0.d0
            dZ_eff_drimp0 = 0.d0
          ...
          ! --- SOL viscosity (tanh transition at psi_n ~ visco_sol_tanh_psin)
          visco_T = visco_T + visco_sol * 0.5d0 * (1.d0 + tanh((psi_norm - visco_sol_tanh_psin) / visco_sol_tanh_sig))
...
!--------------------------------------------------------
          ...
          do im=n_tor_start, n_tor_end
            ...
            ! BB2 spatial derivatives (for centrifugal force terms)
            BB2_x = 2.d0 * (ps0_x * ps0_xx + ps0_y * ps0_xy) / BigR**2 - 2.d0 * BB2 / BigR
            BB2_y = 2.d0 * (ps0_x * ps0_xy + ps0_y * ps0_yy) / BigR**2
            BB2_s = BB2_x * x_s(ms,mt) + BB2_y * y_s(ms,mt)
            BB2_t = BB2_x * x_t(ms,mt) + BB2_y * y_t(ms,mt)
            ...
            rhs_ij(var_psi) = v * eta_T  * (zj0 - current_source(ms,mt) - Jb - jseed0)/ BigR  * xjac * tstep * factor(var_psi,1) &
            ...
            ! Adds ∇(½ρΩ²R²) to J×B = ∇p + ∇(½ρΩ²R²) force balance
            if (with_vpar) then
              C0_cent = 0.5d0 * r0 * vpar0**2 * F0**2 / (BigR**2 * BB2)
              if (abs(vpar0) > 1.d-12) then
                C0_cent_s = C0_cent * (r0_s/r0 + 2.d0*vpar0_s/vpar0 - 2.d0*x_s(ms,mt)/BigR - BB2_s/BB2)
                C0_cent_t = C0_cent * (r0_t/r0 + 2.d0*vpar0_t/vpar0 - 2.d0*x_t(ms,mt)/BigR - BB2_t/BB2)
              else
                C0_cent_s = 0.d0
                C0_cent_t = 0.d0
              endif
              rhs_ij(var_u) = rhs_ij(var_u) &
                - BigR**2 * (v_s * C0_cent_t - v_t * C0_cent_s) * tstep * factor(var_u, 14)
            endif
            ...
            if ( with_vpar ) then
              rhs_ij(var_vpar) = - v * F0 / BigR * P0_p                                                                 * xjac * tstep * factor(var_vpar,1)  &
                                 ...
                                 ! --- Centrifugal parallel force: +b·∇(½ρΩ²R²) = +[ψ, C0]
                                 + v * (C0_cent_s * ps0_t - C0_cent_t * ps0_s)                                                 * tstep * factor(var_vpar,1)  &
            ...
            !###################################################################################################
            !#  Seed Island Source (ECCD-style jseed + psiseed)                                               #
            !#  Compute seed_source_val first, then use in evolution equation or direct RHS                   #
            !###################################################################################################
            if (num_seed_islands > 0) then
              ...
              if (seed_type(1) == 0) then
                !#############################################################################################
                !#  jseed Evolution Equation (ECCD-style)                                                    #
                !#############################################################################################
                if (with_jseed) then
                  Bgrad_jseed = (F0 / BigR * jseed0_p + jseed0_x * ps0_y - jseed0_y * ps0_x) / BigR
                  rhs_ij(var_jseed) =   v * BigR * seed_source_val  * xjac * tstep     &  ! ECCD source
                                      - v * jseed0 * nu_jec_fast * BigR * xjac * tstep &  ! damping
                                      + v * JJ_par * BigR/sqrt(BB2) * Bgrad_jseed * xjac * tstep &  ! convection
                                      + v * zeta * delta_g(mp, var_jseed, ms, mt) * BigR * xjac  ! time-stepping
                end if
              else
                !#############################################################################################
            ...
            if (with_impurities .and. slave_impurity) then
               ! --- Slaved impurity: relaxation source to enforce rimp0 = rimp0_target
               rhs_ij(var_rhoimp)  = -v * (rimp0 - rimp0_target) * slave_impurity_omega * BigR * xjac * tstep
               rhs_ij_k(var_rhoimp) = 0.d0
            else if (with_impurities) then
            ...
            do k=1,n_vertex_max
              ...
              do l=1,n_degrees
                ...
                do in = n_tor_start, n_tor_end
                  ...
                  u    = psi    ;  zj    = psi    ;  w    = psi    ; rho    = psi    ;  Ti    = psi    ; vpar    = psi   ; Te   = psi    ; T   = psi    ;  rhoimp    = psi    ; jseed    = psi    ;
                  u_x  = psi_x  ;  zj_x  = psi_x  ;  w_x  = psi_x  ; rho_x  = psi_x  ;  Ti_x  = psi_x  ; vpar_x  = psi_x ; Te_x = psi_x  ; T_x = psi_x  ;  rhoimp_x  = psi_x  ; jseed_x  = psi_x  ;
                  u_y  = psi_y  ;  zj_y  = psi_y  ;  w_y  = psi_y  ; rho_y  = psi_y  ;  Ti_y  = psi_y  ; vpar_y  = psi_y ; Te_y = psi_y  ; T_y = psi_y  ;  rhoimp_y  = psi_y  ; jseed_y  = psi_y  ;
                  u_p  = psi_p  ;  zj_p  = psi_p  ;  w_p  = psi_p  ; rho_p  = psi_p  ;  Ti_p  = psi_p  ; vpar_p  = psi_p ; Te_p = psi_p  ; T_p = psi_p  ;  rhoimp_p  = psi_p  ; jseed_p  = psi_p  ;
                  u_s  = psi_s  ;  zj_s  = psi_s  ;  w_s  = psi_s  ; rho_s  = psi_s  ;  Ti_s  = psi_s  ; vpar_s  = psi_s ; Te_s = psi_s  ; T_s = psi_s  ;  rhoimp_s  = psi_s  ; jseed_s  = psi_s  ;
                  u_t  = psi_t  ;  zj_t  = psi_t  ;  w_t  = psi_t  ; rho_t  = psi_t  ;  Ti_t  = psi_t  ; vpar_t  = psi_t ; Te_t = psi_t  ; T_t = psi_t  ;  rhoimp_t  = psi_t  ; jseed_t  = psi_t  ;
                  u_ss = psi_ss ;  zj_ss = psi_ss ;  w_ss = psi_ss ; rho_ss = psi_ss ;  Ti_ss = psi_ss ; vpar_ss = psi_ss; Te_ss = psi_ss; T_ss = psi_ss;  rhoimp_ss = psi_ss ; jseed_ss = psi_ss ;
                  u_tt = psi_tt ;  zj_tt = psi_tt ;  w_tt = psi_tt ; rho_tt = psi_tt ;  Ti_tt = psi_tt ; vpar_tt = psi_tt; Te_tt = psi_tt; T_tt = psi_tt;  rhoimp_tt = psi_tt ; jseed_tt = psi_tt ;
                  u_st = psi_st ;  zj_st = psi_st ;  w_st = psi_st ; rho_st = psi_st ;  Ti_st = psi_st ; vpar_st = psi_st; Te_st = psi_st; T_st = psi_st;  rhoimp_st = psi_st ; jseed_st = psi_st ;
                  ...
                  ! --- Centrifugal force Jacobian w.r.t. density (var_rho)
                  ! C0 = 0.5 * r0 * vpar0^2 * F0^2 / (BigR^2 * BB2)
                  ! dC0/dr0 = 0.5 * vpar0^2 * F0^2 / (BigR^2 * BB2)
                  if (with_vpar) then
                    dC0_drho = 0.5d0 * vpar0**2 * F0**2 / (BigR**2 * BB2)
                    if (abs(vpar0) > 1.d-12) then
                      dC0_drho_s = dC0_drho * (2.d0*vpar0_s/vpar0 - 2.d0*x_s(ms,mt)/BigR - BB2_s/BB2)
                      dC0_drho_t = dC0_drho * (2.d0*vpar0_t/vpar0 - 2.d0*x_t(ms,mt)/BigR - BB2_t/BB2)
                    else
                      dC0_drho_s = 0.d0
                      dC0_drho_t = 0.d0
                    endif
                    amat(var_u, var_rho) = amat(var_u, var_rho) &
                      - BigR**2 * (v_s * (dC0_drho * rho_t + dC0_drho_t * rho) &
                                 - v_t * (dC0_drho * rho_s + dC0_drho_s * rho)) * theta * tstep
                  endif
                  ...
                  if (with_vpar) then
                    ...
                    ! --- Centrifugal force Jacobian w.r.t. parallel velocity (var_vpar)
                    ! dC0/dvpar = r0 * vpar0 * F0**2 / (BigR**2 * BB2)
                    if (with_vpar) then
                      dC0_dvpar = r0 * vpar0 * F0**2 / (BigR**2 * BB2)
                      if (abs(vpar0) > 1.d-12) then
                        dC0_dvpar_s = dC0_dvpar * (r0_s/r0 + vpar0_s/vpar0 - 2.d0*x_s(ms,mt)/BigR - BB2_s/BB2)
                        dC0_dvpar_t = dC0_dvpar * (r0_t/r0 + vpar0_t/vpar0 - 2.d0*x_t(ms,mt)/BigR - BB2_t/BB2)
                      else
                        dC0_dvpar_s = 0.d0
                        dC0_dvpar_t = 0.d0
                      endif
                      amat(var_u, var_vpar) = amat(var_u, var_vpar) &
                        - BigR**2 * (v_s * (dC0_dvpar * vpar_t + dC0_dvpar_t * vpar) &
                                   - v_t * (dC0_dvpar * vpar_s + dC0_dvpar_s * vpar)) * theta * tstep
                    endif

                    ! --- Centrifugal force Jacobian w.r.t. psi (var_psi)
                    ! C0 depends on psi through BB2 = (F0^2 + |grad_psi|^2)/R^2.
                    ! For typical tokamaks B_pol/B_phi ~ 0.1, this contributes ~1% of the
                    ! centrifugal term. Currently placeholder; full chain rule needs FEM 2nd derivatives.
                    if (with_vpar) amat(var_u, var_psi) = amat(var_u, var_psi) + 0.d0
                    ! --- Centrifugal force has no Jacobian w.r.t. u, zj, w, T
                    if (with_vpar) amat(var_u, var_u)   = amat(var_u, var_u)   + 0.d0
                    if (with_vpar) amat(var_u, var_zj)  = amat(var_u, var_zj)  + 0.d0
                    if (with_vpar) amat(var_u, var_w)   = amat(var_u, var_w)   + 0.d0
...
!===============================End of new TG_num terms============================
                    ...
                    ! --- Centrifugal parallel force Jacobian w.r.t. density (var_rho)
                    ! Linearization of +[ψ, C0] w.r.t. density
                    ! dC0/dr0 = 0.5 * vpar0^2 * F0^2 / (BigR^2 * BB2)
                    if (with_vpar) then
                      dC0_drho = 0.5d0 * vpar0**2 * F0**2 / (BigR**2 * BB2)
                      if (abs(vpar0) > 1.d-12) then
                      ...
                      amat(var_vpar, var_rho) = amat(var_vpar, var_rho) &
                        + v * (dC0_drho * rho_s * ps0_t - dC0_drho * rho_t * ps0_s) * theta * tstep &
                        + v * (dC0_drho_s * rho * ps0_t - dC0_drho_t * rho * ps0_s) * theta * tstep
                    endif
                    ...
                    !===============================End of new TG_num terms============================
                             ...
                             ! --- Centrifugal parallel force Jacobian w.r.t. parallel velocity (var_vpar)
                             ! Linearization of +[ψ, C0] w.r.t. v_par
                             ! dC0/dvpar = r0 * vpar0 * F0**2 / (BigR**2 * BB2)
                             if (with_vpar) then
                               dC0_dvpar = r0 * vpar0 * F0**2 / (BigR**2 * BB2)
                               if (abs(vpar0) > 1.d-12) then
                               ...
                               amat(var_vpar, var_vpar) = amat(var_vpar, var_vpar) &
                                 + v * (dC0_dvpar * vpar_s * ps0_t - dC0_dvpar * vpar_t * ps0_s) * theta * tstep &
                                 + v * (dC0_dvpar_s * vpar * ps0_t - dC0_dvpar_t * vpar * ps0_s) * theta * tstep
                             endif

                             ! --- Centrifugal parallel force Jacobian w.r.t. psi (var_psi)
                             ! C0 depends on psi through BB2, but B_pol/B_phi ~ 0.1 => ~1% effect.
                             ! Placeholder; full chain rule requires FE 2nd-derivative expansion.
                             if (with_vpar) amat(var_vpar, var_psi) = amat(var_vpar, var_psi) + 0.d0
                             ! --- Centrifugal force has no Jacobian w.r.t. u, zj, w, T
                             if (with_vpar) amat(var_vpar, var_u)  = amat(var_vpar, var_u)  + 0.d0
                             if (with_vpar) amat(var_vpar, var_zj) = amat(var_vpar, var_zj) + 0.d0
                             if (with_vpar) amat(var_vpar, var_w)  = amat(var_vpar, var_w)  + 0.d0
  ...
  !!!!                            -v * Te * (gamma-1.d0) * deta_dT_ohm * (zj0 / BigR)**2.d0 * BigR * xjac * theta * tstep &
                 ...
                 ! in areas where rho is small.   
                  ...
                  if (with_impurities .and. slave_impurity) then
                     ! --- Slaved impurity: relaxation Jacobian only
                     drimp0_target_dr0 = slave_impurity_conc / (slave_impurity_conc + (central_mass/m_imp) * (1.d0 - slave_impurity_conc * Z_imp))
                     amat(var_rhoimp, var_rhoimp)  = -v * rhoimp * slave_impurity_omega * BigR * xjac * theta * tstep
                     amat(var_rhoimp, var_rho)      = +v * rho * drimp0_target_dr0 * slave_impurity_omega * BigR * xjac * theta * tstep
                     amat(var_rhoimp, var_psi)       = 0.d0
                     amat(var_rhoimp, var_u)         = 0.d0
                     if (with_TiTe) then
                        amat(var_rhoimp, var_Ti)     = 0.d0
                        amat(var_rhoimp, var_Te)     = 0.d0
                     else
                        amat(var_rhoimp, var_T)      = 0.d0
                     endif
                     if (with_vpar) amat(var_rhoimp, var_vpar) = 0.d0
                     amat_k(var_rhoimp, var_psi)     = 0.d0
                     amat_k(var_rhoimp, var_vpar)    = 0.d0
                     amat_k(var_rhoimp, var_rhoimp)  = 0.d0
                     amat_n(var_rhoimp, var_vpar)    = 0.d0
                     amat_n(var_rhoimp, var_rhoimp)  = 0.d0
                     amat_kn(var_rhoimp, var_rhoimp) = 0.d0
                  else if (with_impurities) then
                  ...
                  !###################################################################################################
                  !# jseed Jacobian (ECCD-style seed current)                                                        #
                  !###################################################################################################
                  if (with_jseed) then
                    ! --- psi equation w.r.t. jseed (Ohm's law: -eta_T * jseed / R)
                    amat(var_psi, var_jseed) = v * eta_T * jseed / BigR * xjac * theta * tstep

                    ! --- Bgrad_jseed linearizations
                    Bgrad_jseed_psi  = (jseed0_x * psi_y - jseed0_y * psi_x) / BigR
                    Bgrad_jseed_jseed = (F0 / BigR * jseed_p + jseed_x * ps0_y - jseed_y * ps0_x) / BigR

                    ! --- jseed equation w.r.t. psi (parallel convection dependence on psi)
                    amat(var_jseed, var_psi) = - JJ_par * BigR / sqrt(BB2) * v * Bgrad_jseed_psi * xjac * theta * tstep &
                        + JJ_par * BigR * sqrt(BB2) * BB2_psi / (2.d0 * BB2**2) * v * Bgrad_jseed * xjac * theta * tstep

                    ! --- jseed equation diagonal (mass + damping + parallel convection)
                    amat(var_jseed, var_jseed) = v * jseed * BigR * xjac * (1.d0 + zeta) &
                        + v * nu_jec_fast * BigR * jseed * xjac * theta * tstep &
                        - v * JJ_par * BigR / sqrt(BB2) * Bgrad_jseed_jseed * xjac * theta * tstep
                  end if
...
subroutine construct_imp_charge_states()
  ...
  case('Ne')
     ...
     m_imp          = 20.
  case('B')
     m_i_over_m_imp = central_mass/11.
     m_imp          = 11.
  case default
...
subroutine seed_current_source(ps, R, Z, phi, R_axis, Z_axis, n_tor_mode, source_val)
  ...
  use phys_module, only: num_seed_islands, seed_psin, seed_amplitude, seed_width, &
                         ...
                         seed_ramp_steps, seed_type, nu_jec_fast
    ...
    do i = 1, num_seed_islands
      if (seed_q(i) > 0.d0) then
        ...
        if (psi_n_tmp >= 0.d0) then
          ...
          ! Auto-compute jseed amplitude from island width (Rutherford formula)
          if (seed_type(i) == 0 .and. seed_amplitude(i) == 0.d0) then
            seed_amplitude(i) = nu_jec_fast * (seed_width(i) / 4.d0)**2 &
                              * abs(get_dq_dpsi(seed_psin(i))) &
                              * dble(seed_n_tor(i)) * dble(seed_m_pol(i))  ! n*m from Rutherford
          end if
        end if
      end if
    end do
  ...
  ! --- Compute smooth temporal envelope (for psiseed; jseed ignores it)
  ...
  do i = 1, num_seed_islands
    ...
    if (n_tor_mode >= 0) then
      ...
      if (seed_type(i) == 0) then
        source_val = source_val + seed_amplitude(i) * radial_gauss
      else
      ...
      ! FFT mode
      if (seed_type(i) == 0) then
        ! jseed (ECCD-style): pure flux function (no temporal envelope, like model305)
        source_val = source_val + seed_amplitude(i) * radial_gauss
      else if (m_pol > 0) then
        ! psiseed: helical structure cos(m*theta* - n*phi) resonant at q=m/n
