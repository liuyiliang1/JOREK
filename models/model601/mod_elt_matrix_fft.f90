...
real*8     :: source_imp_drift, source_imp_drift_arr(n_inj_max)
  ...
  real*8     :: eta_seed                                              !< CLT-style resistivity perturbation (seed_type=2)
          ...
          do im=n_tor_start, n_tor_end
            ...
            if (num_seed_islands > 0) then
              ...
              if (seed_type == 1) then
                ...
                psi_tilde = seed_ampl_boost * (seed_width(1) / 4.d0)**2 &
                ...
                rhs_ij(var_psi) = rhs_ij(var_psi) &
                  + v * psi_tilde * seed_source_val / BigR * xjac * tstep * factor(var_psi, 5)

              else if (seed_type == 2) then
                ! --- CLT-style: resistivity perturbation inducing local current change
                !     eta_seed = eta10 * Gaussian * cos(m*theta* - n*phi) * t_envelope
                !     Source  = eta_seed * zj0 / BigR  (perturbed electric field, matches CLT: eta1 * (cint+cur))
                eta_seed = seed_eta10 * seed_source_val
                rhs_ij(var_psi) = rhs_ij(var_psi) &
                  + v * eta_seed * zj0 / BigR * xjac * tstep * factor(var_psi, 1)
              end if
            end if
