...
module phys_module
  ...
  real*8  :: seed_amplitude(max_seed_islands) = 0.d0                !< Amplitude of current perturbation for each seed
  ...
  logical :: seed_continuous = .false.                                !< .true.=ECCD-like continuous; .false.=one-time init condition
  integer :: seed_inject_step = 0                                     !< internal: step counter for one-time injection
  integer :: seed_ramp_steps = 10                                      !< number of steps to ramp up psiseed (avoid non-convergence)
  integer :: seed_type(max_seed_islands) = 0                          !< 0=jseed (current source), 1=psiseed (flux perturbation)
  real*8  :: psiseed_width(max_seed_islands) = 0.02d0                 !< island width W in psi_n (psiseed only)
