...
tokamak_name = 'FRC' !'ITER' 'DIII-D' 'JET' 'CFETR' 'HL-2M' 'EAST''EXL-50U','EHL2'
...
if (tokamak_name == 'ITER') then
  ...
  quad_l = 0.0!0.0 !-0.7
  n_tht  = 257
  r0     =  0.74* R_scale !0.8 !0.9
  z0     = -0.0  * R_scale
  a0     = 0.48 * R_scale !0.54 !0.66
...
do i=n_psi+1,n_ext
  psi_ext(i) = 1.d0 + 0.25 * float(i-n_psi)/float(n_sol)
enddo
...
!p(n_psi)=p(n_psi-1)
pres_bnd= p(n_psi)*0.05
if(pres_bnd>0) T_bnd=0
print *,p(n_psi-2:n_psi)
print *,'T_bnd=',T_bnd
dpdpsi_sep = (p(n_psi)-p(n_psi-1))/(psi(n_psi)-psi(n_psi-1))
sig_ext    = (pres_bnd - p(n_psi)) / dpdpsi_sep

p_ext(      1:n_psi) = p(1:n_psi)
p_ext(n_psi+1:n_ext) = (p_ext(n_psi)-pres_bnd)*( 1 - tanh((psi_ext(n_psi+1:n_ext)-psi_ext(n_psi))/sig_ext) ) + pres_bnd
!p_ext=p_ext+p(n_psi-1)
!print*,p_ext(n_psi+1:n_ext)
do i=1,n_ext
