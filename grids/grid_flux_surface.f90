...
subroutine grid_flux_surface_adaptive_tht(xpoint,xcase,node_list,element_list,surface_list, &
                                          n_flux,n_tht,xr1,sig1,xr2,sig2,refinement)
!------------------------------------------------------------------------
! 计算新的通量面网格（基于 HELENA20），支持极向非均匀分布
!------------------------------------------------------------------------
use constants
use tr_module
use data_structure
use mod_neighbours, only: update_neighbours
use mod_interp
use phys_module, only: force_central_node, fix_axis_nodes, treat_axis, XR_tht, SIG_tht, bgf_tht
use equil_info
use mod_grid_conversions
use mod_parameters  
...
real*8,allocatable :: s_values(:),radius(:),psi_values(:),tht_start(:),tht_end(:)
real*8,allocatable :: sp1(:),sp2(:),sp3(:),sp4(:)
real*8,allocatable :: T1(:), T2(:), TP1(:), TP2(:), TP3(:), TP4(:)   ! 极向映射数组
real*8             :: dpsi_ds, dpsi_dss, tht_min, tht_max, rr1, rr2, ss1, ss2
real*8             :: RRg1,dRRg1_dr,dRRg1_ds,dRRg1_drs,dRRg1_drr,dRRg1_dss
real*8             :: ZZg1,dZZg1_dr,dZZg1_ds,dZZg1_drs,dZZg1_drr,dZZg1_dss
real*8             :: RRg2,dRRg2_dr,dRRg2_ds,dRRg2_drs,dRRg2_drr,dRRg2_dss
...
real*8             :: dRRg1_dt, dZZg1_dt, dRRg2_dt, dZZg2_dt, rz0, rz1, rz2, drz1, drz2
real*8             :: a0, a1, a2, a3
real*8             :: theta,theta_val, drr1, dss1, drr2, dss2, t, t2_val, t3_val, ri, si, dri, dsi, check
real*8             :: rad2, th_z, th_r, th_rr, th_zz, th_rz, dth_ds, dth_dr, dth_drs, dth_drr, dth_dss
real*8             :: rzjac, ps_z, ps_r, ejac, ptjac, rt, st, dptjac_dr, dptjac_ds, rpt, spt, rtt, stt
real*8             :: dr_dr, dr_dz, ds_dr, ds_dz, dps_drr, dps_dzz, crr_axis, czz_axis, cx, cy
...
! 极向映射所需变量
real*8, allocatable :: T_uniform(:), T_mapped(:)   ! 均匀坐标和映射后的角度参数（0~1）
real*8              :: theta_uniform, dtheta_dt

write(*,*) '**************************************'
write(*,*) '*     flux surface grid (adaptive)   *'
write(*,*) '**************************************'

my_id = 0

call find_axis(my_id,node_list,element_list,psi_axis_local,R_axis_local,Z_axis_local, &
               i_elm_axis_local,s_axis_local,t_axis_local,ifail)

if (xpoint) call find_xpoint(my_id,node_list,element_list,psi_xpoint_local,R_xpoint_local, &
                             Z_xpoint_local,i_elm_xpoint_local,s_xpoint_local,t_xpoint_local,xcase,ifail)
...
n_max = n_degrees
if (n_order .gt. 5) n_max = (5+1)**2 / 4   ! 高阶忽略
...
if (xpoint) then
  psi_bnd = ES%psi_xpoint(1)
  if (ES%active_xpoint .eq. UPPER_XPOINT) psi_bnd = ES%psi_xpoint(2)
endif
...
! --- 构造极向映射 (非均匀)
allocate(T_uniform(npnew+1), T_mapped(npnew+1))
allocate(TP1(npnew+1), TP2(npnew+1), TP3(npnew+1), TP4(npnew+1))

do j=1,npnew+1
  T_uniform(j) = real(j-1)/real(npnew)
enddo
call meshac2(npnew+1, T_mapped, XR_tht(1), XR_tht(2), SIG_tht(1), SIG_tht(2), bgf_tht, 1.0d0)
call spline(npnew+1, T_uniform, T_mapped, 0.d0, 0.d0, 2, TP1, TP2, TP3, TP4)
...
do i=1,surface_list%n_psi
    ...
    do j=1, npnew
      ...
      ! 极向非均匀映射
      theta_uniform = real(j-1)/real(npnew)
      theta_val=spwert(npnew+1, theta_uniform, TP1, TP2, TP3, TP4, T_uniform, abltg)
      theta = theta_val * 2.d0 * PI        ! 实际物理角度 (0 ~ 2π)
      dtheta_dt = abltg(1) * 2.d0 * PI   ! dθ/dt
      ...
      do k=1, surface_list%flux_surfaces(i)%n_pieces
        if ( (theta .ge. tht_start(k)) .and. (theta .le. tht_end(k)) ) then
          ...
          call SOLVP3(a0,a1,a2,a3,t,t2_val,t3_val,ifail)
          ...
          if (abs(t) .le. 1.d0 + 1.d-6) then
            ...
            ! 极向导数：dR/dθ * dθ/dt → 对均匀坐标 t 的导数
            ! 原均匀公式：RRnew(3) = (- PS_Z / Ejac) / (npnew/PI)
            ! 现调整为 RRnew(3) = (dR/dθ * dθ/dt) / (2*npnew)  (因为 1/(npnew/PI) = PI/npnew，而 PI/npnew = (1/(2*npnew)) * 2π)
            ! 最终等价于 RRnew(3) = dR/dθ * dθ/dt / (2*npnew)
            RRnew(3,npnew*(i) + j) = (- PS_Z / Ejac) * dtheta_dt / (2.d0 * real(npnew))
            ZZnew(3,npnew*(i) + j) = (  PS_R / Ejac) * dtheta_dt / (2.d0 * real(npnew))

            ! 混合导数
            RRnew(4,npnew*(i) + j) =  dr_dpt * dtheta_dt / (2.d0*(nrnew-1)*(npnew)/PI) * dpsi_ds
            ZZnew(4,npnew*(i) + j) =  dz_dpt * dtheta_dt / (2.d0*(nrnew-1)*(npnew)/PI) * dpsi_ds
            ...
            if (n_order .ge. 5) then   ! 忽略高阶，置零
              RRnew (5:n_max,npnew*(i) + j) = 0.d0
              ZZnew (5:n_max,npnew*(i) + j) = 0.d0
              PSInew(5:n_max,npnew*(i) + j) = 0.d0
            endif

          else
            write(*,*) ' T TOO BIG : ',T,T2_val,T3_val
          endif
...
do j=1,npnew
  ...
  theta_uniform = real(j-1)/real(npnew)
  theta_val=spwert(npnew+1, theta_uniform, TP1, TP2, TP3, TP4, T_uniform, abltg)
  theta = theta_val * 2.d0 * PI
  dtheta_dt = abltg(1) * 2.d0 * PI
  ...
  ! 极向导数：需乘以 dtheta_dt 得到对均匀坐标的导数
  RRnew(3,inode) = 0.d0
  ZZnew(3,inode) = 0.d0
  PSInew(1,inode) = ES%psi_axis

  RRnew(2,inode) = RRnew(2,inode) * sp2(1)
  RRnew(3,inode) = RRnew(3,inode) * dtheta_dt
  RRnew(4,inode) = RRnew(4,inode) * sp2(1) * dtheta_dt
  ZZnew(2,inode) = ZZnew(2,inode) * sp2(1)
  ZZnew(3,inode) = ZZnew(3,inode) * dtheta_dt
  ZZnew(4,inode) = ZZnew(4,inode) * sp2(1) * dtheta_dt
  PSInew(2,inode) = PSInew(2,inode) * sp2(1)
  PSInew(3,inode) = 0.d0
  PSInew(4,inode) = PSInew(4,inode) * sp2(1) * dtheta_dt
...
!----------------------------- 转换为 Bezier 网格（与原代码相同）
element_list%n_elements = (nrnew-1)*npnew
node_list%n_nodes       = nrnew*npnew

if ( node_list%n_nodes > n_nodes_max ) then
  write(*,*) 'ERROR in grid_flux_surface_adaptive_tht: n_nodes_max too small'
  stop
else if ( element_list%n_elements > n_elements_max ) then
  write(*,*) 'ERROR in grid_flux_surface_adaptive_tht: n_elements_max too small'
  stop
end if
...
do i=1,nrnew
  do j=1,npnew
    ...
    if (n_order .ge. 5) then   ! 忽略高阶
      node_list%node(index)%X(1,5:n_max,:) = 0.d0
      node_list%node(index)%values(1,5:n_max,1) = 0.d0
    endif
...
do k=n_element_start+1 , element_list%n_elements
  do iv = 1, 4
    ivp = mod(iv,4)   + 1
    ivm = mod(iv+2,4) + 1
    node_iv  = element_list%element(k)%vertex(iv)
...
! 释放临时数组
deallocate(T_uniform, T_mapped, TP1, TP2, TP3, TP4)

return
end subroutine grid_flux_surface_adaptive_tht